...
TH2 *ReverseXAxis2D (TH2 *h)
{
   ...
   newaxis->SetLabelOffset(-0.03);
   //   newaxis->Draw();
   Int_t totBinsX = h->GetXaxis()->GetNbins();
   Int_t totBinsY = h->GetYaxis()->GetNbins();
   
   TH2D *hrev = new TH2D("hrev","hrev", totBinsX, h->GetXaxis()->GetXmin(), h->GetXaxis()->GetXmax(), totBinsY, h->GetYaxis()->GetXmin(), h->GetYaxis()->GetXmax());
   Int_t totBins = totBinsX*totBinsY;
   for(Int_t i=1; i<totBins; i++){
     hrev->SetBinContent(totBins-i+1, h->GetBinContent(i));
     hrev->SetBinError(totBins-i+1, h->GetBinError(i));
   }
...
void plotResidualsUVSV(const char* fname){

  // input: root filename (out of gbl)

  TH2 *ReverseXAxis2D(TH2); 
  ...
  TCanvas *cTop = new TCanvas("cTop","Residuals u vs v in lab frame, TOP", 800, 1200);
  TCanvas *cBot = new TCanvas("cBot","Residuals u vs v in lab frame, BOTTOM",800, 1200);
  ...
  Int_t ipad=0;
  for(Int_t i=1; i<5; i++){
    TString hisname = "residual_after_GBL_vs_v_predicted_module_L"; hisname += i;
    ...
    TString hisnameIni = "residual_after_GBL_vs_v_predicted_module_L"; hisnameIni += i;
    ...
    hisname = "residual_after_GBL_vs_v_predicted_module_L"; hisname += i;
    ...
    hisnameIni = "residual_after_GBL_vs_v_predicted_module_L"; hisnameIni += i;
    ...
    pad->cd();
    if(his) {//ReverseXAxis(his); 
      his->Draw("colz");
      //TH1 *hpfr = (TH1*)ReverseXAxis1D((TH1D*)hpf);
      //hpfr->Draw(); zerol->Draw("same");
      //TH2D *hisr = (TH2D*)ReverseXAxis2D((TH2D*)his); 
      //hisr->Draw("colz");
    }
