...
void plotKinkLambda(const char* fname){
  ...
  Double_t x[20] = {0.9,1.1, 1.9,2.1, 2.9,3.1, 3.9,4.1, 
		    ...
		    5.9,5.95, 6.1,6.15, 
		    6.9,6.95, 7.1,7.15}; 
  TCanvas *cTop = new TCanvas("cTop","Lambda kink in lab frame, TOP", 800, 1200);
  TCanvas *cBot = new TCanvas("cBot","Lambda kink in lab frame, BOTTOM",800, 1200);
  ...
  TString outname ="lambdaKink_"; outname += outn.Data(); outname += ".txt";
  TString topName ="lambdaKink_"; topName += outn.Data(); topName += "_top.gif";
  TString botName ="lambdaKink_"; botName += outn.Data(); botName += "_bot.gif";
  ...
  Int_t ipad=0;
  for(Int_t i=1; i<5; i++){
    TString hisname = "lambda_kink_module_L"; hisname += i;
    ...
    TString hisnameIni = "lambda_kink_module_L"; hisnameIni += i;
    ...
    hisname = "lambda_kink_module_L"; hisname += i;
    ...
    hisnameIni = "lambda_kink_module_L"; hisnameIni += i;
...
//    his->GetXaxis()->SetRangeUser(lo,hi);
    ...
    if(fit){
      cout << "sensor " << i << " stereo mean :  " << his->GetMean()*1. << " - RMS : " << his->GetRMS()*1. << " (rad) " << " " << his->GetEntries() << endl;
      cout << "sensor " << i << " stereo mu :  " << fit->GetParameter(1)*1. << " - sigma : " << fit->GetParameter(2)*1. << " (rad) " << endl;
      outf << layer.Data() << " " 
	   << fit->GetParameter(1)*1. << " " 
	   << fit->GetParError(1)*1. << " " 
	   << fit->GetParameter(2)*1. << " " 
...
//    his->GetXaxis()->SetRangeUser(lo,hi);
    ...
    if(fit){
      cout << "sensor " << i << " axial hole mean :  " << his->GetMean()*1. << " - RMS : " << his->GetRMS()*1. << " (rad) " << " " << his->GetEntries() << endl;
      cout << "sensor " << i << " axial hole mu :  " << fit->GetParameter(1)*1. << " - sigma : " << fit->GetParameter(2)*1. << " (rad) " << endl;
      outf << layer.Data() << " " 
	   << fit->GetParameter(1)*1. << " " 
	   << fit->GetParError(1)*1. << " " 
	   << fit->GetParameter(2)*1. << " " 
    ...
    if(fit){
      cout << "sensor " << i << " stereo hole mean :  " << his->GetMean()*1. << " - RMS : " << his->GetRMS()*1. << " (rad) " << " " << his->GetEntries() << endl;
      cout << "sensor " << i << " stereo hole mu :  " << fit->GetParameter(1)*1. << " - sigma : " << fit->GetParameter(2)*1. << " (rad) " << endl;
      outf << layer.Data() << " " 
	   << fit->GetParameter(1)*1. << " " 
	   << fit->GetParError(1)*1. << " " 
	   << fit->GetParameter(2)*1. << " " 
    ...
    cout << "sensor " << i << " axial slot mean :  " << his->GetMean()*1. << " - RMS : " << his->GetRMS()*1. << " (rad) " << " " << his->GetEntries() << endl;
    if(fit){
      cout << "sensor " << i << " axial slot mu :  " << fit->GetParameter(1)*1. << " - sigma : " << fit->GetParameter(2)*1. << " (rad) " << endl;
      outf << layer.Data() << " " 
	   << fit->GetParameter(1)*1. << " " 
	   << fit->GetParError(1)*1. << " " 
	   << fit->GetParameter(2)*1. << " " 
    ...
    if(fit){
      cout << "sensor " << i << " stereo slot mean :  " << his->GetMean()*1. << " - RMS : " << his->GetRMS()*1. << " (rad) " << " " << his->GetEntries() << endl;
      cout << "sensor " << i << " stereo slot mu :  " << fit->GetParameter(1)*1. << " - sigma : " << fit->GetParameter(2)*1. << " (rad) " << endl;
      outf << layer.Data() << " " 
	   << fit->GetParameter(1)*1. << " " 
	   << fit->GetParError(1)*1. << " " 
	   << fit->GetParameter(2)*1. << " " 
  ...
  for(Int_t i=1; i<5; i++){
    ...
    his->Fit("gaus","Qsame","",low,up);
    TF1 *fit = his->GetFunction("gaus");
    cout << "sensor " << i << " axial mean :  " << his->GetMean()*1. << " - RMS : " << his->GetRMS()*1. << " (rad) " << " " << his->GetEntries() << endl;
    if(fit){
      cout << "sensor " << i << " axial mu :  " << fit->GetParameter(1)*1. << " - sigma : " << fit->GetParameter(2)*1. << " (rad) " << endl;
      outf << layer.Data() << " " 
	   << fit->GetParameter(1)*1. << " " 
	   << fit->GetParError(1)*1. << " " 
	   << fit->GetParameter(2)*1. << " " 
...
//    his->GetXaxis()->SetRangeUser(lo,hi);
  ...
  TCanvas *cmeanAll = new TCanvas("cmeanAll","Lambda kinks, TOP&BOT", 800, 1000);
  cmeanAll->Divide(1,2);
  cmeanAll->cd(1);
  mean1Top->SetMarkerStyle(22);
  mean2Top->SetMarkerStyle(22);
  mean3Top->SetMarkerStyle(22);
  mean1Top->SetMarkerColor(kGray+2);
  ...
  mgTop->GetYaxis()->SetRangeUser(-0.5*1.e-3,0.5*1.e-3);
  mgTop->GetXaxis()->SetTitle("layer");
  mgTop->GetYaxis()->SetTitle("#lambda kink [rad]");
  mgTop->Draw("AP");
  cmeanAll->Update();
  topLeg->Draw("same");
  zero->Draw("same");

  mean1Bot->SetMarkerStyle(22);
  mean2Bot->SetMarkerStyle(22);
  mean3Bot->SetMarkerStyle(22);
  mean1Bot->SetMarkerColor(kGray+2);
  ...
  mgBot->GetYaxis()->SetRangeUser(-0.5*1.e-3,0.5*1.e-3);
  mgBot->GetXaxis()->SetTitle("layer");
  mgBot->GetYaxis()->SetTitle("#lambda kink [rad]");
  mgBot->Draw("AP");
  cmeanAll->Update();
  botLeg->Draw("same");
  zero->Draw("same");
  ...
  TString mean = "lambdaKink_"; mean += outn.Data(); mean += "_TopBot_ini.gif";
  cmeanAll->SaveAs(mean.Data());
				
  TFile *g = new TFile("plotLambdaKink_graphs.root","RECREATE");
  g->cd();
  mgBot->SetName("mgBot");
  mgTop->SetName("mgTop");
  mgTop->Write();
