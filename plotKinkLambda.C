...
void plotKinkLambda(const char* fname){
  ...
  TCanvas *cTop = new TCanvas("cTop","Lambda kink in lab frame, TOP", 800, 1000);
  TCanvas *cBot = new TCanvas("cBot","Lambda kink in lab frame, BOTTOM",800, 1000);
  ...
  TString topName ="lambdaKink_"; topName += outn.Data(); topName += "_topO.gif";
  TString botName ="lambdaKink_"; botName += outn.Data(); botName += "_botO.gif";
  ...
  for(Int_t i=1; i<4; i++){
    ...
    */
    ...
    hisname = "h_corrdiff_lambda_module_L"; hisname  += i;
  ...
  cmeanAll->Divide(1,2);
  cmeanAll->cd(1);
  mean1Top->SetMarkerStyle(21);
  mean2Top->SetMarkerStyle(21);
  mean3Top->SetMarkerStyle(21);
  mean1Top->SetMarkerColor(kBlack);
  mean2Top->SetMarkerColor(kCyan+1);
  mean3Top->SetMarkerColor(kMagenta+1);
  ...
  mgTop->GetYaxis()->SetRangeUser(-1,1);
  mgTop->GetXaxis()->SetTitle("layer");
  mgTop->GetYaxis()->SetTitle("#lambda kink mean value [mrad]");
  mgTop->Draw("AP");
  cmeanAll->Update();
  topLeg->Draw("same");
  zero->Draw("same");

  mean1Bot->SetMarkerStyle(21);
  mean2Bot->SetMarkerStyle(21);
  mean3Bot->SetMarkerStyle(21);
  mean1Bot->SetMarkerColor(kBlack);
  mean2Bot->SetMarkerColor(kCyan+1);
  mean3Bot->SetMarkerColor(kMagenta+1);
  ...
  mgBot->GetYaxis()->SetRangeUser(-1,1);
  mgBot->GetXaxis()->SetTitle("layer");
  mgBot->GetYaxis()->SetTitle("#lambda kink mean value [mrad]");
  mgBot->Draw("AP");
  cmeanAll->Update();
  botLeg->Draw("same");
  zero->Draw("same");
  ...
  TString mean = "mean_"; mean += outn.Data(); mean += "_TopBot_KinkLambda.gif";
  cmeanAll->SaveAs(mean.Data());

  TFile *g = new TFile("lambdaKink__graphs.root","RECREATE");
  g->cd();
  mgBot->SetName("mgBot");
  mgTop->SetName("mgTop");
  mgTop->Write();
