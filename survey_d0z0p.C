...
void survey_d0z0p(const Char_t* filename, Int_t yr=2019){
  TString file=filename;
  TFile *f = new TFile(file.Data());
  gStyle->SetStatH(0.5);  gStyle->SetStatW(0.5);  
  TCanvas *c = new TCanvas("c","c",800,1000);
  TH1F *his1 = (TH1F*)f->Get("d0_top");
  TH1F *his2 = (TH1F*)f->Get("d0_bottom");
  TH1F *his3 = (TH1F*)f->Get("z0_top");
  TH1F *his4 = (TH1F*)f->Get("z0_bottom");
  TH1F *his5 = (TH1F*)f->Get("p_top");
  TH1F *his6 = (TH1F*)f->Get("p_bottom");
  TH1F *his7 = (TH1F*)f->Get("beamspot_x_top");
  TH1F *his8 = (TH1F*)f->Get("beamspot_x_bottom");
  TH1F *his9 = (TH1F*)f->Get("beamspot_y_top");
  TH1F *his10 = (TH1F*)f->Get("beamspot_y_bottom");
  ...
  Int_t pad=1;
  c->cd(pad++);
  Double_t mean_top, mean_bot, meanD0, meanZ0;
  his1->Draw();
  if(his1->Fit("gaus","Q","",-2., 1.)){ 
    TF1 *fit = his1->GetFunction("gaus"); fit->SetLineColor(kBlack);
  ...
  c->cd(pad++); 
  his2->Draw();  
  if(his2->Fit("gaus","Q","",-2., 1.)){
    TF1* fit = his2->GetFunction("gaus"); fit->SetLineColor(kRed);
    TPaveStats *st = (TPaveStats*)his2->FindObject("stats");
    if(st){
      st->SetOptFit(1111); st->SetY1NDC(0.53); st->SetX1NDC(0.53);
    } 
    mean_bot = fit->GetParameter(1);
    meanD0 = mean_top - mean_bot;
  ...
  c->cd(pad++); 
  his3->Draw(); 
  if(his3->Fit("gaus","Q","",-2, 1.)){
    TF1* fit = his3->GetFunction("gaus"); fit->SetLineColor(kBlack);
    TPaveStats* st = (TPaveStats*)his3->FindObject("stats");
  ...
  c->cd(pad++); 
  his4->Draw();
  if(his4->Fit("gaus","Q","",-2., 1.)){
    TF1* fit = his4->GetFunction("gaus"); fit->SetLineColor(kRed);
    TPaveStats* st = (TPaveStats*)his4->FindObject("stats");
    st->SetOptFit(1111); st->SetY1NDC(0.55); st->SetX1NDC(0.55); 
    his4->Draw("same");
    mean_bot = fit->GetParameter(1);
    meanZ0 = mean_top - mean_bot;
  ...
  cout << his5->GetXaxis()->GetBinCenter(maxPeak) - wid << " " << 
  ...
  if(his5->Fit("gaus","Q","",low,up)){
    ...
    TPaveStats *st = (TPaveStats*)his5->FindObject("stats");
    st->SetOptFit(1111); st->SetY1NDC(0.53); st->SetX1NDC(0.53); 
    his5->Draw();
    mean_top = fit->GetParameter(1);
  }
  ...
  c->cd(pad++);
  Double_t meanP;
  if(his6->Fit("gaus","Q","",low,up)){ 
    TF1 *fit = his6->GetFunction("gaus"); fit->SetLineColor(kRed);
    TPaveStats *st = (TPaveStats*)his6->FindObject("stats");
    st->SetOptFit(1111); st->SetY1NDC(0.53); st->SetX1NDC(0.53); 
    his6->Draw();
    mean_bot = fit->GetParameter(1);
    meanP = mean_top - mean_bot;
  ...
  his7->Draw();
  if(his7->Fit("gaus","Q")){
    TF1* fit = his7->GetFunction("gaus"); fit->SetLineColor(kBlack);
    TPaveStats* st = (TPaveStats*)his7->FindObject("stats");
    his7->Draw("same");
    st->SetOptFit(1111); gPad->Update();
  } 
  his8->Draw();
  if(his8->Fit("gaus","Q")){
    TF1* fit = his8->GetFunction("gaus"); fit->SetLineColor(kRed);
    TPaveStats *st = (TPaveStats*)his8->FindObject("stats");
    his8->Draw("same");
    st->SetOptFit(1111); gPad->Update();
  }
  ...
  his9->Draw();
  if(his9->Fit("gaus","Q")){
    TF1* fit = his9->GetFunction("gaus"); fit->SetLineColor(kBlack);
    TPaveStats *st = (TPaveStats*)his9->FindObject("stats");
    his9->Draw("same");
    st->SetOptFit(1111); gPad->Update();
  }
  his10->Draw();
  if(his10->Fit("gaus","")){
    TF1* fit = his10->GetFunction("gaus"); fit->SetLineColor(kRed);
    TPaveStats* st = (TPaveStats*)his10->FindObject("stats");
    his10->Draw("same");
    st->SetOptFit(1111); gPad->Update();
  }
