...
void plotKinkPhi(char* fname){
  ...
  TCanvas *cTop = new TCanvas("cTop","Phi kink in lab frame, TOP", 800, 1000);
  TCanvas *cBot = new TCanvas("cBot","Phi kink in lab frame, BOTTOM",800, 1000);
  ...
  TString topName ="phiKink_"; topName += outn.Data(); topName += "_topO.gif";
  TString botName ="phiKink_"; botName += outn.Data(); botName += "_botO.gif";
  ...
  mgTop->GetYaxis()->SetTitle("#phi kink mean value [mrad]");
  mgTop->Draw("AP");
  cmeanAll->Update();
  topLeg->Draw("same");
  zero->Draw("same");
  ...
  mgBot->GetYaxis()->SetTitle("#phi kink mean value [mrad]");
  mgBot->Draw("AP");
  cmeanAll->Update();
  botLeg->Draw("same");
  zero->Draw("same");
  ...
  TString mean = "mean_"; mean += outn.Data(); mean += "_TopBot_KinkPhi.gif";
  cmeanAll->SaveAs(mean.Data());

  TFile *g = new TFile("phiKink__graphs.root","RECREATE");
  g->cd();
  mgBot->SetName("mgBot");
  mgTop->SetName("mgTop");
  mgTop->Write();
