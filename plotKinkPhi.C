...
void plotKinkPhi(const char* fname){
  ...
  Double_t x[20] = {0.9,1.1, 1.9,2.1, 2.9,3.1, 3.9,4.1, 
		    ...
		    5.9,5.95, 6.1,6.15, 
		    6.9,6.95, 7.1,7.15}; 
  TCanvas *cTop = new TCanvas("cTop","Phi kink in lab frame, TOP", 800, 1200);
  TCanvas *cBot = new TCanvas("cBot","Phi kink in lab frame, BOTTOM",800, 1200);
  ...
  for(Int_t i=1; i<5; i++){
    ...
    TString hisnameIni = "phi_kink_module_L"; hisnameIni += i;
    ...
    hisnameIni = "phi_kink_module_L"; hisnameIni += i;
...
//    his->GetXaxis()->SetRangeUser(lo,hi);
  ...
  TCanvas *cmeanAll = new TCanvas("cmeanAll","Phi kinks, TOP&BOT", 800, 1000);
  cmeanAll->Divide(1,2);
  cmeanAll->cd(1);
  mean1Top->SetMarkerStyle(23);
  mean2Top->SetMarkerStyle(23);
  mean3Top->SetMarkerStyle(23);
  mean1Top->SetMarkerColor(kGray+3);
  mean2Top->SetMarkerColor(kCyan-7);
  mean3Top->SetMarkerColor(kMagenta-7);
  ...
  mgTop->GetYaxis()->SetTitle("#phi kink [rad]");
  mgTop->Draw("AP");
  cmeanAll->Update();
  topLeg->Draw("same");
  zero->Draw("same");

  mean1Bot->SetMarkerStyle(23);
  mean2Bot->SetMarkerStyle(23);
  mean3Bot->SetMarkerStyle(23);
  mean1Bot->SetMarkerColor(kGray+3);
  mean2Bot->SetMarkerColor(kCyan-7);
  mean3Bot->SetMarkerColor(kMagenta-7);
  ...
  mgBot->GetYaxis()->SetTitle("#phi kink [rad]");
  mgBot->Draw("AP");
  cmeanAll->Update();
  botLeg->Draw("same");
  zero->Draw("same");
