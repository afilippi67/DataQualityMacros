...
void plotResidualsUVSPosVProfile_new(char* fname){
  ...
  TLine *zerol = new TLine(-55, 0., 55, 0.);
  ...
  TString outname ="residualsUvsV_"; outname += outn.Data(); outname += "_profile.txt";
  TString topName ="residualsUvsV_"; topName += outn.Data(); topName += "_profile_topO.gif";
  TString botName ="residualsUvsV_"; botName += outn.Data(); botName += "_profile_botO.gif";
  ...
  for(Int_t i=1; i<4; i++){
    ...
    his->Fit("gaus","Q0","",low,up); 
    TH1 *hpfrev = (TH1*)ReverseXAxis(hpf); 
    //hpfrev->Draw();
    hpf->Draw(); zerol->Draw("same");
